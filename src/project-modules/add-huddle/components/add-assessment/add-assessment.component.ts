...
import { BsModalRef, esLocale, defineLocale } from 'ngx-bootstrap';
...
export class AddAssessmentComponent implements OnInit {
  ...
  constructor(private modalService:BsModalService, private router: ActivatedRoute, 
    ...
    private headerService: HeaderService, private localeService: BsLocaleService) { 
		this.subscription = this.headerService.languageTranslation$.subscribe(languageTranslation => {
		
				this.translation = languageTranslation;
				console.log("translation from assessment",this.translation)
				// if(this.translation.huddle_title) this.huddle_data.title = this.translation.huddle_title;
				this.huddle_data.title = this.translation.huddle_new_assessment_huddle
				// this.setTitle(3);
  ...
  ngOnInit() {
    ...
    this.huddle_data.huddle_type=3;
  ...
  private RunSubscribers(){
      ...
      console.log('all users are',this.selecedusers)
  ...
  private formatEditableHuddle(data){
	  ...
	  this.huddle_data.huddle_type = 3;//Number(data.huddle_type);
  ...
  private LoadHuddleTypes(){
	...
	public submitHuddle(){
		...
		let json = this.prepareHuddleData("assessment");
		...
		if(this.EditMode){
			...
			this.mainService.EditHuddle(json).subscribe((data: any) => {
				...
				if (data.success) {
					//location.href = environment.baseUrl + "/Huddles/view/"+this.params.huddle_id;
		...
		this.mainService.AddHuddle(json).subscribe((data: any) => {
			...
			if(data.success){
				location.href = environment.baseUrl + "/Huddles/view/"+data.huddle_id;
			}else{
				this.toastr.info(this.translation.huddle_sorry_for_inconvenience);
				//location.reload();
			}
    ...
    // type.id=3;
		...
		this.huddle_data.huddle_type = 3;//type.id;
