...
export class AddCollaborationComponent implements OnInit {
  ...
  constructor(private modalService: BsModalService, private router: Router, private activatedRoute: ActivatedRoute,
    ...
    private headerService: HeaderService, private localeService: BsLocaleService,public detailsService: DetailsHttpService) {
  ...
  ngOnInit() {
    ...
    this.huddle_data.huddle_type = 1;
  ...
  private formatEditableHuddle(data) {
    ...
    this.huddle_data.huddle_type = 1;//Number(data.huddle_type); // huddle type added b ejaz 8-july-2019
    ...
    if (data.chk_frameworks) {
      ...
      if (data.allow_download_recordings) {
        if (Number(data.allow_download_recordings) == 1) { 
				this.huddle_data.more_settings.allow_download_recordings = true;
				this.moreSettingsToPublish.allow_download_recordings = true;
      }
    }
  ...
  private assignFixRole(user?, role_id?) {
    ...
    // huddle type added b ejaz 8-july-2019
    this.huddle_data.huddle_type = 1;
    ...
    let interval_id = setInterval(() => {
      ...
      if (headerData && headerData.user_current_account) {
        ...
        let obj = {
          ...
          huddle_type: 1//typeId//that.mainService.GetFilter("huddle_type")
        };
  ...
  public submitHuddle() {
    ...
    let json = this.prepareHuddleData("collaboration");
  ...
  public selectType(type) {
    ...
    this.mainService.UpdateFilter("huddle_type", 1);

    if (!this.EditMode)
      this.getUsers(1, false, true);

    this.huddle_data.huddle_type = 1 //type.id; // huddle type added b ejaz 8-july-2019
