...
export class GroupSearchPipe implements PipeTransform {
  ...
  transform(items,term, keys?, huddleType?, sortBy?): any {

    if (!items)
         return items;
         return this.filter(items, term||"", keys?keys.split(","):false, huddleType, sortBy);
   }
 
 
   private filter(items,term, keys?, huddleType?, sortBy?){
