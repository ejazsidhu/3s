...
import { MainService, PlayerService, ScrollService, CropPlayerService } from '@videoDetail/services';
...
//    (this.VideoInfo.h_type==2 || this.VideoInfo.h_type==3) && (this.permissions.coaching_summary_check || this.permissions.assessment_summary_check);
  ...
  private handleVideo(data){
    ...
    this.isAuthenticatedUser = true;
  	this.src = {"path":data.stream_url, "type":"application/x-mpegURL"};
    //this.getNewserverresponse(this.src.path);
    ...
    setTimeout(()=>{
        if (Hls && Hls.isSupported() && this.loaded) {
            ...
            if(this.video)
            {
                ...
                console.log("hls",this.video );
                this.hls.on(Hls.Events.MANIFEST_PARSED, () => {
                    this.video.play();
                    console.log("video.play()",Hls.Events.MANIFEST_PARSED);
                    console.log("video.play()",Hls.Events);
                    this.video.play();
  	...
  	let that = this;
  	this.live_viewer_interval = setInterval(function () {
    that.mainService.GetLiveViewerCount({'stream_name':that.VideoInfo.stream_name,'streamer_url':data.streamer_url}).subscribe((data:any) => {
            console.log("Live viewer count = ",data);
            that.viewers_count = data.data;
        })
...
let a=timeArray;
    ...
    if(reference_id == this.params.video_id){
      ...
      if(commentObj.created_by == this.header_data.user_current_account.User.id || commentObj.active == 1){
        ...
        if(!commentObj.parent_comment_id){
            ...
            this.LoadRubricsTabData(); // update performance level data
  ...
  private processCommentEdited(comment, from_cake, reference_id) {
    if(reference_id == this.params.video_id){
      ...
      this.toastr.success(this.translation.note_updated);
    }
  }
  ...
  private processCommentDeleted(parent_comment_id, item_id, reference_id ) {
    ...
    if(reference_id == this.params.video_id){
      ...
      this.updateCommentCount();
      this.mainService.ReRenderMarkers.emit(true);
      this.toastr.info(this.translation.note_deleted);
    }
  ...
  //   this.socket_listener = this.socketService.EventData.subscribe(res =>{
  //     console.log("websocket res",res)
  ...
  //       case "comment_added":
  //         if(res.is_reply){
  //           this.processCommentReplyAdded(res.data);
  //         }else{
  ...
  //         break;
  //       case "comment_deleted":
  //         this.processCommentDeleted(res.data);
  //         //console.log(data)
  //         break;
  //       case "resource_added":
  ...
  //         //   this.processAttachmentAdded(data.data[0]);
  //         //   console.log(data)
  //         //   break;
  //         case "attachment_deleted":
  ...
  //   this.comments.forEach((o,i) => {
  //     if(o.id == comment.id){
  //       comment.valid=true;
  //       this.comments[i] = comment
  //     }else if(comment.parent_comment_id == o.id){ //this part is for Reply Edit
  //       o.Comment.responses.forEach((r,i) => {
  //         if(r.id == comment.id){
  ...
  // private processCommentDeleted(comment) {
  ...
  // }
  // private processResourceAdded(comment) {
  //        this.staticFiles.push(comment);
  // }
  // private processAttachmentDeleted(comment) {
  //   this.staticFiles.forEach((x,i) => {
  //     if(x.doc_id == comment){
  //        this.staticFiles.splice(i,1);
  //       }
  //   });
  // }
  // private processCommentReplyAdded(comment) {
  //   //debugger;
  //   let data:any = comment;
  //   data.valid = true;
  //   this.comments.forEach((x,i) => {
  //     if(x.id == data.parent_comment_id){
  //       if (!x.Comment) x.Comment = {};
  //       if (!x.Comment.responses) x.Comment.responses = [];
  //       x.Comment.responses.push(data)
  //     }
  //   });
  // }

  // private socketPushFunctionComment() {
  //   console.log("params and videos before",`huddle-details-${this.account_folder_id}`);
  //   //console.log("params with videos",this.params.video_id);
  //   if(this.account_folder_id){
  //     console.log("params and videos",`huddle-details-${this.account_folder_id}`);
  //     this.socketService.pushEvent(
  //       `huddle-details-${this.account_folder_id}`
  //     );
  //   }
    
  //   console.log(this.video_id)
  //   this.socketService.pushEvent(
  //     `video-details-${this.video_id}`
  //   );
  // }
  private processResourceRenamed(data, is_dummy,res?) {
    if(data.doc_type == '1' && (data.id == this.params.video_id || data.doc_id == this.params.video_id)) { 
      ...
      if(res && res.live_video_event && res.live_video_event == "1"){
      this.toastr.info(this.translation.live_recording_stopped_redirect);
    setTimeout(() => {
      this.router.navigate([`/video_huddles/huddle/details/${this.account_folder_id}/artifacts/grid`]);
}, 4000);
}
