...
import { AudioRecorderHelper } from '@videoDetail/helpers';
...
// added to remove conflict
  public mainService:any;
  // public mainService:MainService
  constructor(private modalService :BsModalService, private scrollToService:ScrollService, private toastr:ToastrService, private headerService:HeaderService, private router: Router, private route:ActivatedRoute, private playerService:PlayerService, private cropPlayerService:CropPlayerService,private deviceService: DeviceDetectorService, private socketService:SocketService, private sanitizer: DomSanitizer) { 
  ...
  private LoadPublishSettings(){
    ...
    this.mainService.GetPublishSettings(obj).subscribe((data:any)=>{
  ...
  public onReplyEdit(commentObj){
    ...
    let sessionData:any = this.headerService.getStaticHeaderData();
        ...
        this.mainService.EditTextComment(obj).subscribe((data:any)=>{
  ...
  public UploadResource(file, wholeVideo?, commentId?){
    ...
    this.mainService.UploadResource(obj).subscribe((data:any)=>{
  ...
  public LoadRubricsTabData(callOnSocket?, selectedStandardId?, selectedRatingId?){
    ...
    this.mainService.GetPLTabData(obj).subscribe((data)=>{
...
//    (this.VideoInfo.h_type==2 || this.VideoInfo.h_type==3) && (this.permissions.coaching_summary_check || this.permissions.assessment_summary_check);
    ...
    if(this.EditMode){
      ...
      if (ParsedDataFromLs) {
        ...
        if(ParsedDataFromLs.time) {
          ...
          this.VideoCurrentTime = 'All Video';
        }
      }
    }
  ...
  if(!comment.id || comment.id==""){
    ...
    let obj = {
     ...
     this.mainService.DeleteComment(obj).subscribe((data:any)=>{
       if(data.status=="success"){
    ...
    let allowComment = this.allowToComment()
  	if(allowComment){
        ...
        this.ResetForm();
  			...
  			this.mainService.AddTextComment(obj).subscribe((data:any)=>{
...
let a=timeArray;
  ...
  async onStop() {
    ...
    this.audio = await this.audioRecorder.stop();
    this.audio.audioUrl = this.sanitizer.bypassSecurityTrustUrl(this.audio.audioUrl);
  ...
  private async initializeRecordAudio()  {
    this.audioRecorder = await new  AudioRecorderHelper().recordAudio();
    // recorder.start();
    // await sleep(3000);
    // setTimeout(async () => {
