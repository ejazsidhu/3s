...
import { MainService, PlayerService } from "../../services";
...
export class VideoPlayerComponent implements OnChanges, OnDestroy {
  ...
  /** Video auto comment play variable start */
  ...
  /** Video auto comment play variable end */
  ...
  constructor(private headerService: HeaderService, private mainService: MainService, private cd: ChangeDetectorRef, private playerService: PlayerService,private http: HttpClient, private appMainService: AppMainService) {
  ...
  private RunSubscribers() {
    ...
    // setInterval(()=>{ Todo: high check and fix that why we need to run detectChanges in interval
  ...
  ngAfterViewInit() {
    ...
    this.player = videojs(videoT, {
      ...
      preload: false,
      fullscreenToggle: true,
      playbackRates: [0.5, 1, 1.25, 1.5, 2, 5]
      // ,
      // flash: {
